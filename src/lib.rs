...
pub struct MockHeader {
    ...
    pub source_headers: Vec<PathBuf>,
...
impl Mocksmith {
        ...
        P: AsRef<Path>,
    {
        ...
        mocks.iter_mut().for_each(|m| {
            m.parent_header = Some(file.as_ref().to_path_buf());
        });
        Ok(mocks)
    }
    ...
    {
        let index = clang::Index::new(&self.clang, true, false);
        let tus: Vec<TranslationUnit> = files
            .iter()
        ...
        header.source_headers = files.iter().map(|f| f.as_ref().to_path_buf()).collect();
        Ok(header)
    }
