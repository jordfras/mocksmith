...
pub fn default_name_output_file(header: &MockHeader) -> String {
    ...
    // the stem
    if header.source_headers.len() == 1 {
        if let Some(stem) = header.source_headers[0].file_stem() {
...
mod tests {
    ...
    fn default_name_output_file_uses_mock_name_when_only_one_mock() {
        let info = MockHeader {
            source_headers: vec![std::path::PathBuf::from("source.h")],
    ...
    fn default_name_output_file_uses_extension_from_source_file() {
        let info = MockHeader {
            source_headers: vec![std::path::PathBuf::from("source.hpp")],
            parent_names: vec!["ISomething".to_string()],
            names: vec!["MockSomething".to_string()],
            code: String::new(),
        };
